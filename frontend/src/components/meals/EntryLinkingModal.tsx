    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-semibold text-gray-800">Link Entries to {meal.name}</h3>
              ...
              {linkedEntries.map((entry) => (
                ...
                >
                  <div className="text-sm">
                    ...
                    <div className="text-gray-600">{formatDate(entry.createdAt)}</div>
                    {entry.notes != null && (
                      <div className="text-gray-600 text-xs">Notes: {entry.notes}</div>
                    )}
                  </div>
                  <button
        ...
        {unlinkedEntries.length === 0 && linkedEntries.length === 0 && (
          <div className="text-gray-500 text-center py-4">No entries available to link.</div>
        )}
