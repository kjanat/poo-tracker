...
export function RecentEntries({ entries, loading }: RecentEntriesProps): ReactElement {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      {entries.length === 0 ? (
        <div className="text-gray-500 text-center py-8">No entries yet. Add your first entry!</div>
      ) : (
        <div className="space-y-3">
            ...
            <div key={entry.id} className="border-b border-gray-100 pb-3 last:border-b-0">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  {entry.volume != null && (
                    <span className="text-sm text-gray-600 ml-2">Volume: {entry.volume}</span>
                  )}
                  {entry.color != null && (
                    <span className="text-sm text-gray-600 ml-2">Color: {entry.color}</span>
                  )}
                </div>
              ...
              {entry.notes != null && entry.notes !== '' && (
                <div className="text-sm text-gray-600 mt-1">Notes: {entry.notes}</div>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="mt-4 text-center">
        <a href="/entries" className="text-amber-600 hover:text-amber-700 text-sm font-medium">
          View all entries →
        </a>
      </div>
    </div>
